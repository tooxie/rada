...
class LastFM(object):
    ...
    def get_artist(self, name):
        ...
        if not artist:
            logger.debug('[ Last.FM ] Retrieving artist "%s"' % name)
    ...
    def get_artist_image(self, name):
        ...
        logger.debug('[ Last.FM ] Retrieving artist image for "%s"' % name)
        with ignored(Exception):
    ...
    def get_album(self, name, artist_name):
        ...
        if not album:
            ...
            logger.debug('[ Last.FM ] Retrieving album "%s" by "%s"' % (
                name, artist.name))
            with ignored(Exception):
    ...
    def get_release_date(self, album_name, artist_name):
        ...
        logger.debug('[ Last.FM ] album "%s" by "%s" release date' % (
    ...
    def get_album_cover(self, album_name, artist_name):
        ...
        logger.debug('[ Last.FM ] Retrieving album "%s" by "%s" cover image' % (
                  album_name, artist_name))
        with ignored(Exception):
